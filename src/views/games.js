...
function gameCard(summary) {
  ...
  return el(
    ...
    el(
      ...
      el('span', { text: summary.playerNames.join(' · ') }),
      el('br'),
      el('span', {
    ...
    el(
      ...
      el('button', {
        ...
        style: { 'min-height': '2.625rem', 'font-size': '0.9375rem' },
      ...
      el('button', {
        ...
        style: { 'min-height': '2.625rem', flex: '0 0 auto' },
        'aria-label': `${t('common.delete')} — ${summary.name}`,
...
export function renderGames() {
  ...
  if (summaries.length === 0) {
    root.appendChild(el('div', { style: { 'margin-top': '1rem' } }, emptyState(t('games.empty'))));
    return root;
  }

  const list = panel();
  for (const summary of summaries) list.appendChild(gameCard(summary));
  root.appendChild(el('div', { class: 'stagger', style: { 'margin-top': '1rem' } }, list));
