...
export function syncEntries(game) {
  ...
  for (const round of game.rounds) {
    ...
    for (const id of ids) {
      if (!round.entries[id]) round.entries[id] = makeEntry();
    }
...
/** True when a round's result counts towards the standings. */
export function roundCounts(round) {
  return round.phase === PHASE.DONE;
}
...
export function isComplete(game) {
  return game.rounds.length > 0 && currentRoundIndex(game) < 0;
}
...
/**
 * The bid the dealer is forbidden from making, or null when the rule is off,
 * the dealer is sitting out, or somebody else still has to bid.
 *
...
export function forbiddenDealerBid(game, round) {
  ...
  const dealerEntry = entryOf(round, dealerId);
  if (!dealerEntry || dealerEntry.out) return null;
  let others = 0;
  for (const id of activePlayerIds(game, round)) {
...
export function roundIndexById(game, roundId) {
  return game.rounds.findIndex((r) => r.id === roundId);
}
...
export function setRoundPhase(game, index, phase) {
  const round = game.rounds[index];
  if (!round) return false;
  round.phase = phase;
  normalizeDealers(game);
...
export function sanitizeGame(game) {
  ...
  const repairNumber = (key, lo, hi) => {
    const raw = Number(game.cfg[key]);
    game.cfg[key] = Number.isFinite(raw) ? clamp(Math.round(raw), lo, hi) : cfgDefaults[key];
  };
  repairNumber('banFrom', 0, MAX_CARDS);
  for (const k of ['ptsBid', 'ptsTrick', 'ptsMiss']) repairNumber(k, -99, 99);
  game.planSpec = { ...defaultPlanSpec(), ...(game.planSpec || {}) };
  ...
  game.rounds = (Array.isArray(game.rounds) ? game.rounds : [])
    ...
    .map((r) => {
      ...
      return {
        ...
        phase: phases.has(r.phase) ? r.phase : PHASE.PENDING,
        entries,
      };
    });
