...
import { PALETTE, PHASE, SCHEMA_VERSION, sanitizeGame, uid } from './model.js';
...
export function packGame(game) {
  ...
  return [
    ...
    game.players.map((p) => {
      ...
      return [p.name, idx >= 0 ? idx : p.color, p.carryIn || 0];
    }),
...
export function unpackGame(data) {
  ...
  const players = playersArr.map((p, i) => {
    const [pname, color, carryIn] = Array.isArray(p) ? p : [];
    return {
      id: uid('p'),
  ...
  const rounds = roundsArr.map((r) => {
    const [cards, phaseCode, dealerSeat, dealerLocked, seats] = Array.isArray(r) ? r : [];
    const entries = {};
...
export function readSharePayload(input) {
  ...
  for (const part of fragment.split('&')) {
    ...
    if (eq < 0) continue;
    if (part.slice(0, eq) === PAYLOAD_KEY) return part.slice(eq + 1) || null;
  }
  return null;
}
