...
test('csvField leaves negative numbers importable as numbers', () => {
  ...
  // "+5" is never a value this app emits, so it can only be a player name —
  // and Excel would evaluate it. Escaped, unlike a genuine number.
  assert.equal(csvField('+5'), "'+5");
});

test('csvField will not let a crafted name break the row structure', () => {
  // Number() accepts surrounding whitespace, so Number("5\r\n") is 5. Letting
  // that through unquoted would split one row into two and corrupt the export.
  assert.equal(csvField('5\r\n'), '"5\r\n"');
  assert.equal(csvField('5\n6'), '"5\n6"');
  // Surrounding spaces are harmless to the row structure, so they stay bare.
  assert.equal(csvField(' 5 '), ' 5 ');
  assert.equal(toCsv([['a', '5\r\n', 'b'], ['c', 'd', 'e']]).split('\r\n').length, 3, 'quoted, so the row survives');
});
