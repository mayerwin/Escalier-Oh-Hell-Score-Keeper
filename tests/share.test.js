...
test('readSharePayload finds the payload in URLs and bare fragments', () => {
  ...
  // A messaging client may re-serialise the link and percent-encode it.
  assert.equal(readSharePayload('#g%3DABC'), null, 'the key itself must still be readable');
  assert.equal(readSharePayload('#g=AB%2DC'), 'AB-C');
...
test('an oversized payload is refused before anything is built from it', async () => {
  // deflate compresses this wire format around 200x, so a small link can
  // declare millions of players. Materialising that locked the tab and ate
  // gigabytes before sanitizeGame ever got to apply its caps.
  const bomb = [
    M.SCHEMA_VERSION,
    'bomb',
    Date.now(),
    0,
    [5, 5, 5, 0, 3],
    Array.from({ length: 50_000 }, () => ['x', 0, 0]),
    0,
    [],
  ];
  const raw = `B${bytesToBase64Url(new TextEncoder().encode(JSON.stringify(bomb)))}`;
  const started = Date.now();
  await assert.rejects(() => decodePayload(raw), /too many players|payload too large/);
  assert.ok(Date.now() - started < 2000, 'refused promptly rather than after building it');
});

test('a payload declaring absurdly many rounds is refused', async () => {
  const many = [
    M.SCHEMA_VERSION,
    'bomb',
    Date.now(),
    ...
    [['Ana', 0, 0]],
    0,
    Array.from({ length: 5000 }, () => [1, 0, 0, 0, [[0, 0, 0, 0]]]),
  ];
  const raw = `B${bytesToBase64Url(new TextEncoder().encode(JSON.stringify(many)))}`;
  await assert.rejects(() => decodePayload(raw), /too many rounds|payload too large/);
});

test('a legitimate long game still fits inside the caps', async () => {
  const game = playedGame({ players: 8, rounds: 30 });
  const restored = await decodePayload(await encodeGame(game));
  assert.equal(restored.rounds.length, 30);
